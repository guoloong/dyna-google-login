...
Server-side Google OAuth 2.0 sign-in for the WooCommerce **My Account** page on [dyna-nutrition.com](https://www.dyna-nutrition.com). No JavaScript SDK, no Composer, no third-party runtime services.

A focused, dependency-free plugin that adds a "Continue with Google" button to `/my-account/`. Designed to work behind reverse proxies (Cloudflare, nginx) and with custom themes (Divi, etc.) that override the default WooCommerce login form.
...
- ✅ **Tested** — 20 regression tests covering JWT verification, account linking, and the Cloudflare HTTPS-detection flow
...
5. Copy the **Authorized redirect URI** shown on the settings page into your OAuth client's redirect URIs.
...
Tests stub the WordPress functions used by the plugin and exercise the security-critical paths (JWT verification, account linking, username collisions, HTTPS detection) plus UI rendering. **20/20 tests should pass.**
